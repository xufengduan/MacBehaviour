...
tokenCheckOne <- function (data,max_tokens,systemPrompt,n){
  ...
  token_sum = sum(tokens_vector)
  token_max = max(tokens_vector)

  result <- data.frame(
    CheckItem = c("item numbers", "max_token_numbers", "sum_token_numbers"),
    Values = c(nrow(data),token_max, token_sum)
  )
...
tokenCheckRun <- function (data,max_tokens,systemPrompt){
  ####################################################################
  show <- data.frame(Run = numeric(0), sum_tokens_per_run = numeric(0))
  ...
  for (r in unique(data$Run)) {
    ...
    sum_token_per_run = nrow(r_data) * (max_tokens + prompt_token_sum)+as.numeric(tiktokenTokenizer(list(systemPrompt))[1])
    new_row <- data.frame(Run = r, sum_tokens_per_run = sum_token_per_run)
...
#' @param model The chatGPT model version to be used, default is "gpt-3.5-turbo-1106".
...
#'                      model="gpt-3.5-turbo-1106",max_tokens=10,temperature=0.7,top_p=1,n=1)
#'
...
#'
#' }
preCheck <- function (data, checkToken=F,systemPrompt=" ",
                      model="gpt-3.5-turbo-1106",max_tokens=500,temperature=0.7,top_p=1,n=1){
  ...
  if(oneTrialMode){
    ...
    gptConfig=list(data,systemPrompt=systemPrompt,model=model,max_tokens=max_tokens,temperature=temperature,top_p=top_p,n=n)
    return(gptConfig)
  }
    ...
    gptConfig=list(data,systemPrompt=systemPrompt,model=model,max_tokens=max_tokens,temperature=temperature,top_p=top_p,n=1)
    return(gptConfig)
  }
}
